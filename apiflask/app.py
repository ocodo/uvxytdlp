...
def should_refresh_cache() -> bool:
    ...
    try:
        ...
        if datetime.now() - last_refresh_time > REFRESH_INTERVAL:
            logger.info(f"More than {REFRESH_INTERVAL} passed since last refresh. Refreshing cache.")
            return True
    except ValueError:
...
# --- APIFlask Schema for Input Validation ---
...
def download_via_ytdlp(json_data):
    ...
    # Check for uvx
    if not (os.path.exists(UVX_EXPECTED_PATH) and os.access(UVX_EXPECTED_PATH, os.X_OK)):
    ...
    try:
        ...
        return f"Error: Invalid yt-dlp arguments format: {e}", 400, {'Content-Type': 'text/plain'}


    full_command = uvx_command_parts + ['yt-dlp'] + parsed_yt_dlp_args + [url]
    if download_dir:
        full_command = uvx_command_parts + ['yt-dlp', '-o', f"{download_dir}/%(title)s.%(ext)s"] + parsed_yt_dlp_args + [url]


    logger.info(f"Executing command: {' '.join(shlex.quote(part) for part in full_command)}")
    ...
    try:
        ...
        if process.returncode != 0:
            output_log += f"--- yt-dlp process exited with code {process.returncode} ---\n"
            logger.warning(f"yt-dlp process for {url} exited with code {process.returncode}. stderr: {process.stderr[:500]}")
